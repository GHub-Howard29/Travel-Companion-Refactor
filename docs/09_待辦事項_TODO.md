...
> 目前開發版本：V3.3.1（尚未驗證、尚未發布）
>
> V3.4.0 維持暫不啟動，待 V3.3.1 完成驗證與發布決策後再安排。
>
> 最後更新：2026-07-22
...
## V3.3.1 行程 Day 預設與工具排序（已實作，尚未驗證／發布）

- [x] 旅程進入時依出發日期預設選取今天對應的 Day。
- [x] 編輯旅程時可調整共同檢查清單、私人確認清單、領隊／導遊聯絡資訊與其他資訊的順序。
- [x] 排序設定寫回 Trip 側欄資料；未登入仍隱藏私人確認清單。
- [x] `npm run build` 通過。
- [ ] Product Owner 回歸：旅程開始日、中間日、結束日與非旅程期間的預設 Day。
- [ ] Product Owner 回歸：調整工具順序後儲存、重新整理、重新進入及切換旅程。
- [ ] Product Owner 回歸：未登入時私人確認清單隱藏，其他三項工具順序正常。
- [ ] Product Owner 決定並執行 V3.3.1 發布／部署流程；目前尚未發布。

版本控制：

- V3.3.1 在驗證與發布決策完成前，不啟動 V3.4.0。

---

## V3.3.0 外幣換算（已完成驗證，待手動發布）

開始條件：

- [x] V3.2.1 已於 2026-07-20 正式發布。
...
- [x] Product Owner 已完成換匯紀錄、臺銀參考匯率、離線使用、雙估算、Trip 隔離、權限與跨瀏覽器同步回歸。
- [x] 已完成 V3.3.0 程式與文件提交。
- [ ] Product Owner 手動執行 V3.3.0 發布／部署流程。

版本鎖定規則：

- 在 Product Owner 完成 V3.3.0 手動發布前，不得開啟 V3.4.0 或其他新版本的功能開發。
- 本版本期間的小修正、缺陷修復與驗證調整全部掛在 V3.3.0，不另開版本。
- 只有 V3.3.0 已完成手動發布後，才能開始 V3.4.0。

---

## V3.4.0 前端載入效能優化（已預先規劃，尚未開始）

啟動條件：

- [x] V3.3.0 外幣換算主功能回歸全部通過。
- [x] V3.3.0 無已知需改善問題。
- [ ] V3.3.0 待 Product Owner 手動正式發布。

預定工作：

- [ ] 盤點超過 500 kB 的初始 JavaScript bundle。
- [ ] 將合適的非首屏功能改為動態載入，降低首次開啟下載量。
- [ ] 驗證 PWA 離線快取、首次開啟與工具切換維持正常。

---

## V3 Foundation 重構（既有完成項目）
...
- Other Info / 特殊資訊刪除同步、重複儲存防護與單一網址標題超連結顯示已完成並驗證
- 檢查清單模組已完成第一階段拆分
...
- [x] 領隊導遊聯絡資訊 / 自駕租車須知與其他資訊管理入口一致化：預設為瀏覽模式，有權限者先點「管理」才進入新增、編輯、刪除層，管理層需提供「退出」按鈕。
- [x] 旅費記帳本編輯模式補上既有附件單獨刪除功能：刪除附件時保留帳目資料，只移除附件與附件連結，並更新待同步狀態。
- [x] 修正手機新增帳目附件入口：行動裝置需同時支援使用手機拍照與選擇既有照片 / 檔案，不可只跳出單一路徑。
- [x] 上述三項完成後，需同步回寫 `docs/03_Bug修正紀錄.md` 的狀態與本交接文件。

---

## 近期高優先待辦（2026-07-12）

- [x] 版本資訊分流：本次更新內容不再重複列入版本歷史。
- [x] 版本更新提示只在 App / PWA 安裝模式顯示，瀏覽器網頁開啟不提示更新。
- [x] 手機瀏覽器未安裝 App 時顯示安裝建議，提示 3 秒後自動收合。
- [x] 登入前新增安全提示，說明只使用 Google Email 與登入狀態，不讀取 Gmail、Drive、通訊錄、照片或行事曆。
- [x] 領隊 / 開車特殊資訊頁補上管理入口。
- [x] 記帳本附件入口改為「拍照」與「相簿」兩個明確按鈕。
...
- [x] V3.1.3 針對 iOS PWA Google 兩步驗證改用專用 OAuth 開啟流程，並補上 YouTube / Google App 驗證時的操作提示。
- [x] V3.1.3 針對 iOS 照片同步改為 Blob 重新包裝與 ArrayBuffer 重試，上傳失敗時顯示實際錯誤原因。
- [x] V3.1.4 恢復非強制更新提示，提供馬上更新 / 稍後更新，馬上更新會清除 App 暫存後重新載入。
- [x] V3.1.4 將新增 / 編輯旅程的參與者與登入 Email 合併為 `名稱=Email` 欄位，並補上可編輯者 Email 權限說明。
- [x] V3.1.4 幣別選單新增並排序為 TWD、JPY、KRW、USD、EUR，並同步記帳本可選幣別。
- [x] V3.1.4 修正共同帳本分攤邏輯，TWD / JPY / KRW 採整數進位，零頭盈餘由代墊者承接。
- [x] V3.1.5 補強 Android PWA 馬上更新流程，加入 reload fallback。
- [x] V3.1.5 開放 iOS Safari 網頁模式顯示版本更新提示。
...
- [ ] 實機確認 V3.1.4 非強制更新提示、馬上更新清暫存、稍後更新再次提醒。
- [ ] 實機確認新增旅程 `名稱=Email` 欄位、可編輯者 Email 權限、TWD / JPY / KRW 分攤結算。
- [ ] 實機確認 Android PWA 點馬上更新後會重新載入到 V3.1.5。
- [ ] 實機確認 iOS Safari 網頁模式可看到版本更新提示。
...
- [x] 新增帳目與編輯帳目的附件入口需一致支援手機拍照與相簿 / 檔案選擇。
- [x] 附件同步失敗後重試不得清除尚未成功上傳的本機暫存。
- [x] 多人共用帳本需支援 realtime / 固定時間刷新，避免另一台手機看不到最新帳目。
...
- [x] 天數產生 Day 1 ~ Day N
- [x] 參與者作為帳本付款人來源
- [x] 參與者可對應登入 Email，記帳本付款人依登入帳號鎖定
- [x] 新增 / 編輯旅程 UI 已整併為 `名稱=Email` 欄位，避免參與者名稱與登入 Email 重複輸入
