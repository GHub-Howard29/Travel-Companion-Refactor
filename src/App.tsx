...
interface StoredExpenseItem_old extends ExpenseItem {
  created_at?: string;
}

interface AdminUser_old {
  email: string;
  role: 'super_admin' | 'trip_editor';
  trip_id: string | null;
}

interface EditExpenseDraft_old {
  title: string;
  amount: string;
  payer: string;
  currency: string;
...
interface LocalAttachmentRecord_old {
  id: string;
  expenseId: string;
  tripId: string;
  fileName: string;
...
const isRecord_old = (value: unknown): value is Record<string, unknown> => {
  return typeof value === 'object' && value !== null;
};

const readStorageArray_old = (key: string): unknown[] => {
...
const getStoredExpensesForTrip_old = (tripId: string, fallbackCurrency: string): StoredExpenseItem[] => {
...
const toBookStorageKey_old = (bookTripId: string) => {
  return `cached_expenses_${bookTripId}`;
};

const toPersonalBookTripId_old = (tripId: string, email: string) => {
  return `${tripId}::personal::${email.trim().toLowerCase()}`;
};

const isPersonalBookTripId_old = (tripId: string) => {
